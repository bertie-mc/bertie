...
import io.github.bertie_mc.creatures.server.entity.ai.*;
...
import io.github.bertie_mc.creatures.server.message.MountedEntityKeyMessage;
...
public class TremorsaurusEntity extends DinosaurEntity
    ...
    private static final EntityDataAccessor<Boolean> RUNNING =
            SynchedEntityData.defineId(TremorsaurusEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> HELD_MOB_ID =
            SynchedEntityData.defineId(TremorsaurusEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> TAME_ATTEMPTS =
            SynchedEntityData.defineId(TremorsaurusEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Float> METER_AMOUNT =
            SynchedEntityData.defineId(TremorsaurusEntity.class, EntityDataSerializers.FLOAT);
    public final LegSolver legSolver = new LegSolver(
            new LegSolver.Leg(-0.45F, 0.75F, 1.0F, false), new LegSolver.Leg(-0.45F, -0.75F, 1.0F, false));
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(
                ...
                new TemptGoal(
                        this,
                        1.1D,
                        Ingredient.of(ACBlockRegistry.COOKED_DINOSAUR_CHOP.get(), ACBlockRegistry.DINOSAUR_CHOP.get()),
                        false));
        ...
        this.targetSelector.addGoal(
                3, new MobTargetUntamedGoal<>(this, GrottoceratopsEntity.class, 100, true, false, null));
    }
        ...
        if (!level().isClientSide) {
            ...
            if (riderHitEntity != null
                ...
                if (this.hasLineOfSight(riderHitEntity)
                    ...
                    riderHitEntity.hurt(riderHitEntity.damageSources().mobAttack(this), (float)
                            this.getAttribute(Attributes.ATTACK_DAMAGE).getValue());
                    if (riderHitEntity instanceof LivingEntity living) {
                        living.knockback(
                                0.5D, this.getX() - riderHitEntity.getX(), this.getZ() - riderHitEntity.getZ());
                    }
                    riderHitEntity = null;
                }
            }
            LivingEntity target =
                    riderHitEntity instanceof LivingEntity ? (LivingEntity) riderHitEntity : this.getTarget();
            ...
            Player player = BertieCreatures.PROXY.getClientSidePlayer();
            if (player != null && player.isPassengerOfSameVehicle(this)) {
                if (BertieCreatures.PROXY.isKeyDown(2) && getMeterAmount() >= 1.0F) {
                    BertieCreatures.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 2));
                }
                if (BertieCreatures.PROXY.isKeyDown(3)
                        && (this.getAnimation() == NO_ANIMATION || this.getAnimation() == null)) {
                    BertieCreatures.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 3));
                }
            }
        }
        if (this.isVehicle()) {
    ...
    private void scareMobs() {
        ...
        List<LivingEntity> list = this.level()
                .getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(30, 10, 30));
            ...
            for (int x = -radius; x <= radius; x++) {
                for (int z = -radius; z <= radius; z++) {
                    if (x * x + z * z <= radius * radius) {
                        ...
                        if (water > 0.0F) {
                            level().addParticle(
                                            ACParticleRegistry.WATER_TREMOR.get(),
                                            mutableBlockPos.getX() + 0.5F,
                                            mutableBlockPos.getY() + water + 0.01,
                                            mutableBlockPos.getZ() + 0.5F,
                                            0,
                                            0,
                                            0);
                        }
    ...
    public boolean wantsToAttack(LivingEntity living, LivingEntity owner) {
        if (living instanceof TremorsaurusEntity tremorsaurus
                && (tremorsaurus.getTameAttempts() > 0 || tremorsaurus.hasEffect(ACEffectRegistry.STUNNED))) {
            return false;
        }
    ...
    public void positionRider(Entity passenger, MoveFunction moveFunction) {
        if (this.isPassengerOfSameVehicle(passenger)
            ...
            moveFunction.accept(
                    ...
                    this.getY() + seatOffset.y + ridingOffset - maxLegSolverHeight,
    ...
    public boolean onFeedMixture(ItemStack itemStack, Player player) {
        if (itemStack.is(net.minecraft.world.item.Items.GOLDEN_CARROT) && this.hasEffect(ACEffectRegistry.STUNNED)) {
            this.removeEffect(ACEffectRegistry.STUNNED);
            BertieCreatures.sendMSGToAll(
                    new UpdateEffectVisualityEntityMessage(this.getId(), this.getId(), 3, 0, true));
            if (!level().isClientSide) {
    ...
    public boolean isFood(ItemStack stack) {
        return this.isTame()
                && (stack.is(ACBlockRegistry.COOKED_DINOSAUR_CHOP.get().asItem())
            ...
            if (type == 3) {
                if (this.getAnimation() == NO_ANIMATION || this.getAnimation() == null) {
                    HitResult hitresult = ProjectileUtil.getHitResultOnViewVector(
                            keyPresser, entity -> !entity.is(this) && !this.isAlliedTo(entity), 10.0F);
