...
public class CrushedBlockEntity extends AbstractMovingBlockEntity {
    ...
    private static final EntityDataAccessor<Float> CRUSH_PROGRESS =
            SynchedEntityData.defineId(CrushedBlockEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> DROP_CHANCE =
            SynchedEntityData.defineId(CrushedBlockEntity.class, EntityDataSerializers.FLOAT);
            ...
            if (!level().isClientSide && !droppedItems) {
                for (MovingBlockData dataBlock : this.getData()) {
                    ...
                    if (level().random.nextFloat() < this.getDropChance()) {
                        ...
                        while (level().getBlockState(dropAtPos).isAir()
                                && dropAtPos.getY() > level().getMinBuildHeight()) {
