...
import io.github.bertie_mc.creatures.server.entity.util.TephraExplosion;
...
public class TephraEntity extends Projectile {
    ...
    private static final EntityDataAccessor<Optional<UUID>> ARC_TOWARDS_ENTITY_UUID =
            SynchedEntityData.defineId(TephraEntity.class, EntityDataSerializers.OPTIONAL_UUID);
    private static final EntityDataAccessor<Float> MAX_SCALE =
            SynchedEntityData.defineId(TephraEntity.class, EntityDataSerializers.FLOAT);
    ...
    public void tick() {
        ...
        if (!playedSpawnSound) {
            ...
            this.playSound(
                    ACSoundRegistry.TEPHRA_WHISTLE.get(),
                    8.0F,
                    Mth.clamp(2.0F - this.getMaxScale() * 0.5F, 0.5F, 2.0F));
        }
        if (!level().isClientSide) {
            ...
            if (arcTowards != null
                    ...
                    && tickCount < 20) {
                Vec3 arcVec = arcTowards
                        .position()
                        .add(0, 0.3 * arcTowards.getBbHeight(), 0)
            ...
            for (int j = 0; j < 1 + random.nextInt(2); ++j) {
                Vec3 delta = new Vec3(random.nextFloat() - 0.5F, random.nextFloat() - 0.5F, random.nextFloat() - 0.5F)
                        .scale(0.025F);
                this.level()
                        .addAlwaysVisibleParticle(
                                ACParticleRegistry.TEPHRA.get(),
                                true,
                                this.getRandomX(this.getScale()) + this.getDeltaMovement().x,
                                this.getRandomY() + this.getDeltaMovement().y,
                                this.getRandomZ(this.getScale()) + this.getDeltaMovement().z,
                                delta.x,
                                delta.y,
                                delta.z);
            }
    ...
    private void explode() {
        Explosion.BlockInteraction blockinteraction =
                net.neoforged.neoforge.event.EventHooks.canEntityGrief(level(), this)
                        ? level().getGameRules().getBoolean(GameRules.RULE_MOB_EXPLOSION_DROP_DECAY)
                                ? Explosion.BlockInteraction.DESTROY_WITH_DECAY
                                : Explosion.BlockInteraction.DESTROY
                        : Explosion.BlockInteraction.KEEP;
        TephraExplosion explosion = new TephraExplosion(
                level(), this, this.getX(), this.getY(0.5), this.getZ(), 1.0F + getMaxScale(), blockinteraction);
        explosion.explode();
