...
import io.github.bertie_mc.creatures.server.entity.living.DeepOneMageEntity;
...
public class DeepOneReactToPlayerGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (worldTime != 0 && deepOne.getRandom().nextInt(15) != 0
                || (target != null && target.isAlive())
                || deepOne.getCorneringPlayer() != null) {
            return false;
        }
        ...
        if (!list.isEmpty()) {
            ...
            for (Player scanningPlayer : list) {
                if ((closest == null
                                || scanningPlayer.distanceToSqr(deepOne) < closest.distanceToSqr(deepOne)
                                || deepOne.getReputationOf(scanningPlayer.getUUID()) > highestReputation)
                        && deepOne.hasLineOfSight(scanningPlayer)) {
            ...
            return player != null
                    && (reaction1 != DeepOneReaction.AGGRESSIVE || deepOne.isSummoned() || !player.isCreative())
                    && reaction1.validPlayer(deepOne, player);
        }
        return false;
    }
    ...
    public boolean canContinueToUse() {
        ...
        return player != null
                ...
                && reaction != null
                && deepOne.getCorneringPlayer() == null
                && deepOne.distanceTo(player) >= reaction.getMinDistance()
                && deepOne.distanceTo(player) <= reaction.getMaxDistance()
                && (attackTarget == null || !attackTarget.isAlive())
                && (reaction != DeepOneReaction.AGGRESSIVE || !player.isCreative() || deepOne.isSummoned())
                && !deepOne.isTradingLocked()
                && reaction.validPlayer(deepOne, player);
    }
    ...
    private void tickFollow(float propensity) {
        float f = 0.1F;
        if (player.getOffhandItem().is(ACTagRegistry.DEEP_ONE_BARTERS)
                || player.getMainHandItem().is(ACTagRegistry.DEEP_ONE_BARTERS)) {
            f = 0.2F;
        }
        ...
        if (friendlyLookAtTime > 0) {
            deepOne.getLookControl()
                    .setLookAt(this.player.getX(), this.player.getEyeY(), this.player.getZ(), 10.0F, (float)
                            this.deepOne.getMaxHeadXRot());
            friendlyLookAtTime--;
        }
        if (following) {
            if (distance < 4) {
    ...
    private void tickStalking() {
        ...
        if (chaseTime >= (deepOne.getLastHurtByMob() == player ? 10 : 60)) {
            ...
            if (isBeingLookedAt || deepOne.getRandom().nextInt(100) == 0) {
                ...
                while ((moveTarget == null || moveTarget.distanceTo(deepOne.position()) < 3) && j < 10) {
                    Vec3 vec3 = DefaultRandomPos.getPosTowards(
                            deepOne, 15, 15, player.position(), (double) ((float) Math.PI / 2F));
                    if (vec3 != null) {
                        float mageUp = deepOne instanceof DeepOneMageEntity
                                ? 1 + deepOne.getRandom().nextInt(2)
                                : 0;
            ...
            if (moveTarget != null
                    && moveTarget.distanceTo(deepOne.position()) > 3
                    && (isBeingLookedAt || distance >= 12)) {
                ...
                deepOne.getNavigation()
                        .moveTo(moveTarget.x, moveTarget.y + mageUp, moveTarget.z, isBeingLookedAt ? 2F : 1F);
            }
        }
    }
    ...
    private boolean isEntityLookingAt(LivingEntity looker, LivingEntity seen, double degree) {
        ...
        Vec3 vec31 = new Vec3(
                seen.getX() - looker.getX(),
                seen.getBoundingBox().minY
                        + (double) seen.getEyeHeight()
                        - (looker.getY() + (double) looker.getEyeHeight()),
                seen.getZ() - looker.getZ());
