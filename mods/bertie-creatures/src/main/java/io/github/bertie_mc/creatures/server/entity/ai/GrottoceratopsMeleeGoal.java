...
public class GrottoceratopsMeleeGoal extends Goal {
    ...
    public void tick() {
        ...
        if (target != null) {
            ...
            if (dist < grottoceratops.getBbWidth() + target.getBbWidth() + 3.0D) {
                if (grottoceratops.getAnimation() == IAnimatedEntity.NO_ANIMATION) {
                    if (grottoceratops.getRandom().nextBoolean()) {
                        ...
                        tryAnimation(
                                left
                                        ? GrottoceratopsEntity.ANIMATION_MELEE_TAIL_1
                                        : GrottoceratopsEntity.ANIMATION_MELEE_TAIL_2);
                    }
                }
            }
            ...
            if (grottoceratops.getAnimation() == GrottoceratopsEntity.ANIMATION_MELEE_TAIL_1
                    || grottoceratops.getAnimation() == GrottoceratopsEntity.ANIMATION_MELEE_TAIL_2) {
                if (grottoceratops.getAnimationTick() > 10 && grottoceratops.getAnimationTick() <= 12) {
    ...
    private void checkAndDealDamage(LivingEntity target, float multiplier) {
        if (grottoceratops.hasLineOfSight(target)
                && grottoceratops.distanceTo(target) < grottoceratops.getBbWidth() + target.getBbWidth() + 1.0D) {
            grottoceratops.playSound(ACSoundRegistry.GROTTOCERATOPS_ATTACK.get());
            target.hurt(
                    target.damageSources().mobAttack(grottoceratops),
                    (float) grottoceratops
                                    .getAttribute(Attributes.ATTACK_DAMAGE)
                                    .getValue()
                            * multiplier);
            target.knockback(
                    0.8D + 0.5D * multiplier,
                    grottoceratops.getX() - target.getX(),
                    grottoceratops.getZ() - target.getZ());
        }
    }
