...
public class AlexsCavesChestBoatEntity extends ChestBoat implements AlexsCavesBoat {
    ...
    private static final EntityDataAccessor<Integer> AC_BOAT_TYPE =
            SynchedEntityData.defineId(AlexsCavesChestBoatEntity.class, EntityDataSerializers.INT);
    ...
    protected void readAdditionalSaveData(CompoundTag nbt) {
        ...
        if (nbt.contains("ACBoatType")) {
            this.entityData.set(
                    AC_BOAT_TYPE,
                    AlexsCavesBoat.Type.byName(nbt.getString("ACBoatType")).ordinal());
        }
    }
            ...
            if (onGround) {
                if (this.fallDistance > 3.0F) {
                    ...
                    this.causeFallDamage(
                            this.fallDistance, 1.0F, this.damageSources().fall());
                    if (!this.level().isClientSide && !this.isRemoved()) {
                        this.kill();
                        if (this.level().getGameRules().getBoolean(GameRules.RULE_DOENTITYDROPS)) {
                            for (int i = 0; i < 3; ++i) {
                                this.spawnAtLocation(
                                        this.getACBoatType().getPlankSupplier().get());
                            }
