...
import io.github.bertie_mc.creatures.server.entity.util.LaysEggs;
import io.github.bertie_mc.creatures.server.entity.util.RidingMeterMount;
...
public abstract class DinosaurEntity extends TamableAnimal
        implements IDancesToJukebox, LaysEggs, IAdvancedPathingMob, RidingMeterMount {
    ...
    public static boolean checkPrehistoricSpawnRules(
            EntityType<? extends Animal> type,
            LevelAccessor levelAccessor,
            MobSpawnType mobType,
            BlockPos pos,
            RandomSource randomSource) {
        return levelAccessor.getBlockState(pos.below()).is(ACTagRegistry.DINOSAURS_SPAWNABLE_ON)
                && levelAccessor.getFluidState(pos).isEmpty()
                && levelAccessor.getFluidState(pos.below()).isEmpty();
    }

    public static boolean checkPrehistoricPostBossSpawnRules(
            EntityType<? extends Animal> type,
            LevelAccessor levelAccessor,
            MobSpawnType mobType,
            BlockPos pos,
    ...
    public void handleEntityEvent(byte b) {
        if (b == 77) {
            ...
            for (int i1 = 0; i1 < particleCount; i1++) {
                ...
                BlockPos ground = BlockPos.containing(ACMath.getGroundBelowPosition(
                        level(),
                        new Vec3(
                                Mth.floor(this.getX() + extraX),
                                Mth.floor(this.getY() + extraY),
                                Mth.floor(this.getZ() + extraZ))));
    ...
    public BlockState createEggBeddingBlockState() {
        return net.minecraft.world.level.block.Blocks.HAY_BLOCK.defaultBlockState();
    }

    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ItemStack itemstack = player.getItemInHand(hand);
        if (this instanceof TremorsaurusEntity
                && itemstack.is(net.minecraft.world.item.Items.GOLDEN_CARROT)
                && this.hasEffect(io.github.bertie_mc.creatures.server.potion.ACEffectRegistry.STUNNED)) {
            if (!level().isClientSide && onFeedMixture(itemstack, player)) {
                ...
                if (canOwnerCommand(player)) {
                    ...
                    player.displayClientMessage(
                            Component.translatable(
                                    "entity.bertiecreatures.all.command_" + this.getCommand(), this.getName()),
                            true);
    ...
    public boolean startRiding(Entity entity, boolean force) {
        ...
        if (flag && entity instanceof AbstractMinecart) {
            ...
            for (DinosaurEntity dinosaur : this.level()
                    .getEntitiesOfClass(
                            DinosaurEntity.class,
                            this.getBoundingBox().inflate(advancementRange, advancementRange, advancementRange))) {
                if (dinosaur.getRootVehicle() instanceof AbstractMinecart
                        && !nearbyDinosaurEntityTypes.contains(dinosaur.getType())) {
            ...
            if (nearbyDinosaurEntityTypes.size() >= 5) {
                for (Player player : level().getEntitiesOfClass(
                                Player.class, this.getBoundingBox().inflate(advancementRange))) {
