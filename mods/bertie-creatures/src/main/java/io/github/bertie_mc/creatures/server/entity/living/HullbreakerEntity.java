...
import io.github.bertie_mc.creatures.server.entity.ai.AnimalRandomlySwimGoal;
import io.github.bertie_mc.creatures.server.entity.ai.HullbreakerInspectMobGoal;
import io.github.bertie_mc.creatures.server.entity.ai.HullbreakerMeleeGoal;
import io.github.bertie_mc.creatures.server.entity.ai.VerticalSwimmingMoveControl;
...
public class HullbreakerEntity extends WaterAnimal implements IAnimatedEntity, KaijuMob {
    ...
    private static final EntityDataAccessor<Integer> INTEREST_LEVEL =
            SynchedEntityData.defineId(HullbreakerEntity.class, EntityDataSerializers.INT);
    ...
    public static boolean checkHullbreakerSpawnRules(
    ...
    public ItemEntity spawnAtLocation(ItemStack stack) {
        ...
        if (itementity != null) {
            if (this.headPart != null) {
                ...
                Vec3 mouth = this.headPart
                        .position()
                        .add(yOnlyViewVector.scale(-0.5F))
                        .add(0, 0.5F, 0);
                ...
                itementity.setDeltaMovement(yOnlyViewVector
                        .add(
                                ...
                                random.nextFloat() * 0.2F - 0.1F)
                        .normalize()
                        .scale(0.8F + level().random.nextFloat() * 0.3F));
            }
    ...
    protected void tickDeath() {
        ...
        if (!level().isClientSide) {
            ...
            if (this.getAnimation() == ANIMATION_DIE
                    && this.getAnimationTick() > 10
                    && this.getAnimationTick() % 7 == 0
                    && collectedLoot
                    && !deathItems.isEmpty()) {
    ...
    public void tick() {
        ...
        float targetFishPitch =
                Mth.clamp((float) this.getDeltaMovement().y * 2F, -1.4F, 1.4F) * -(float) (180F / (float) Math.PI);
        if (!isAlive()) {
        ...
        if (this.getAnimation() == HullbreakerEntity.ANIMATION_BASH
                && this.getAnimationTick() > 10
    ...
    public void breakBlock() {
        ...
        if (!level().isClientSide
            ...
            for (int a = (int) Math.round(damageBox.minX); a <= (int) Math.round(damageBox.maxX); a++) {
                for (int b = (int) Math.round(damageBox.minY) - 1;
                        (b <= (int) Math.round(damageBox.maxY) + 1) && (b <= 127);
                        b++) {
    ...
    private void tickMultipart() {
        ...
        this.headPart.setPosCenteredY(
                this.rotateOffsetVec(new Vec3(0, 0, 3.5F), fishPitch + this.getXRot(), this.getYHeadRot())
                        .add(center));
    ...
    private double swimDegree(float width, float sinOffset) {
        double move =
                Math.cos(this.walkAnimation.position() * 0.33F + sinOffset) * this.walkAnimation.speed() * width * 0.8F;
        double idle =
                Math.sin((tickCount + BertieCreatures.PROXY.getPartialTicks()) * 0.05F + sinOffset) * width * 0.5F;
        return (move + idle * (1 - this.walkAnimation.speed()))
                * (1 - getLandProgress(BertieCreatures.PROXY.getPartialTicks()));
    }
    ...
    protected SoundEvent getAmbientSound() {
        return isInWaterOrBubble()
                ? ACSoundRegistry.HULLBREAKER_IDLE.get()
                : ACSoundRegistry.HULLBREAKER_LAND_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return isInWaterOrBubble()
                ? ACSoundRegistry.HULLBREAKER_HURT.get()
                : ACSoundRegistry.HULLBREAKER_LAND_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return isInWaterOrBubble()
                ? ACSoundRegistry.HULLBREAKER_DEATH.get()
