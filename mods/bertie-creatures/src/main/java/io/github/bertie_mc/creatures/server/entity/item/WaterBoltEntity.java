...
public class WaterBoltEntity extends Projectile {
    ...
    private static final EntityDataAccessor<Optional<UUID>> ARC_TOWARDS_ENTITY_UUID =
            SynchedEntityData.defineId(WaterBoltEntity.class, EntityDataSerializers.OPTIONAL_UUID);
    private static final EntityDataAccessor<Boolean> BUBBLING =
            SynchedEntityData.defineId(WaterBoltEntity.class, EntityDataSerializers.BOOLEAN);
    private int lSteps;
    private double lx;
    private double ly;
    private double lz;
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            ...
            if (arcTowards != null
                    && (tickCount > 3 || seekAmount > 0.3)
                    && dieIn == -1
                    && this.distanceTo(arcTowards) > 1.5F
                    && (tickCount < 20 || seekAmount > 0.3 && tickCount < 40)) {
                Vec3 arcVec = arcTowards
                        .position()
                        .add(0, 0.85F * arcTowards.getBbHeight(), 0)
                        .subtract(this.position())
                        .normalize();
                float prevDeltaScale = 1.0F - (seekAmount - 0.3F) * 0.3F;
                this.setDeltaMovement(
                        this.getDeltaMovement().scale(prevDeltaScale).add(arcVec.scale(seekAmount)));
            }
        } else {
            for (int j = 0; j < 3 + random.nextInt(2); ++j) {
                this.level()
                        .addParticle(
                                this.isInWaterOrBubble() || isBubbling()
    ...
    private void damageMobs() {
        ...
        DamageSource source =
                damageSources().mobProjectile(this, (owner instanceof LivingEntity living1 ? living1 : null));
        ...
        for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, bashBox)) {
            if (!isAlliedTo(entity)
                    && !(entity instanceof DeepOneBaseEntity)
    ...
    private void onRicochetHit(Entity hitBy) {
        ...
        if (ricochetTo != null && owner instanceof LivingEntity living) {
            ...
            Vec3 arcVec = ricochetTo
                    .position()
                    .add(0, 0.3F + 1 * ricochetTo.getBbHeight(), 0)
                    .subtract(this.position())
                    .normalize();
