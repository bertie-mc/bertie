...
public class WaveEntity extends Entity {
    ...
    private static final EntityDataAccessor<Boolean> SLAMMING =
            SynchedEntityData.defineId(WaveEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    private static final EntityDataAccessor<Integer> WAITING_TICKS =
            SynchedEntityData.defineId(WaveEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Float> Y_ROT =
            SynchedEntityData.defineId(WaveEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> WAVE_SCALE =
            SynchedEntityData.defineId(WaveEntity.class, EntityDataSerializers.FLOAT);
        ...
        if (this.level().isClientSide) {
            ...
            for (int particleCount = 0; particleCount < getWaveScale(); particleCount++) {
                for (int i = 0; i <= 4; i++) {
                    ...
                    spawnParticleAt(
                            (0.2F + random.nextFloat() * 0.2F) * this.getWaveScale(),
                            1.2F,
                            xOffset * 1.2F * this.getWaveScale(),
                            ACParticleRegistry.WATER_FOAM.get());
                    spawnParticleAt(
                            (0.2F + random.nextFloat() * 0.2F) * this.getWaveScale(),
                            -0.2F,
                            xOffset * 1.4F * this.getWaveScale(),
    ...
    private void attackEntities(float scale) {
        ...
        for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, bashBox)) {
            if (!isAlliedTo(entity)
                ...
                this.setSlamming(true);
                entity.knockback(
                        0.1D + 0.5D * scale, (double) Mth.sin(this.getYRot() * ((float) Math.PI / 180F)), (double)
