...
public class TremorsaurusMeleeGoal extends Goal {
            ...
            if (dist < tremorsaurus.getBbWidth() + target.getBbWidth() + 1.0D) {
                if (tremorsaurus.getAnimation() == IAnimatedEntity.NO_ANIMATION) {
                    if ((tremorsaurus.getRandom().nextBoolean()
                                            || Math.max(target.getBbHeight(), target.getBbWidth()) >= 2.0F)
                                    && !grab
                            || tremorsaurus.isBaby()) {
    ...
    private void checkAndDealDamage(LivingEntity target) {
        if (tremorsaurus.hasLineOfSight(target)
            ...
            target.hurt(target.damageSources().mobAttack(tremorsaurus), (float)
                    tremorsaurus.getAttribute(Attributes.ATTACK_DAMAGE).getValue());
