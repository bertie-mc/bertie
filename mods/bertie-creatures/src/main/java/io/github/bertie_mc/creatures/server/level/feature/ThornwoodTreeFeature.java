package io.github.bertie_mc.creatures.server.level.feature;

import com.mojang.serialization.Codec;
import io.github.bertie_mc.creatures.server.block.ACBlockRegistry;
import io.github.bertie_mc.creatures.server.block.ThornwoodBranchBlock;
...
public class ThornwoodTreeFeature extends Feature<NoneFeatureConfiguration> {
        ...
        if (centerAboveGround > 0) {
            ...
            for (Direction direction : ACMath.HORIZONTAL_DIRECTIONS) {
                if (rootCount <= 3 + randomsource.nextInt(1)) {
                    generateRoot(
                            level,
                            centerPos.relative(direction),
                            0.25F,
                            randomsource,
                            direction,
                            centerAboveGround + 1 + randomsource.nextInt(6));
                    rootCount++;
                }
            }
        }
        ...
        while (i < tallPart) {
            ...
            if (randomsource.nextInt(5) == 0) {
                ...
                level.setBlock(
                        trunkPos,
                        i == tallPart
                                ? ACBlockRegistry.THORNWOOD_WOOD.get().defaultBlockState()
                                : ACBlockRegistry.THORNWOOD_LOG.get().defaultBlockState(),
                        3);
            }
        ...
        for (Direction direction : ACMath.HORIZONTAL_DIRECTIONS) {
            ...
            for (int j = 1; j <= canopyLength; j++) {
                ...
                if (randomsource.nextInt(2) != 0) {
                    ...
                    level.setBlock(
                            canopyLogPos,
                            ACBlockRegistry.THORNWOOD_WOOD
                                    .get()
                                    .defaultBlockState()
                ...
                if (j == canopyLength && level.getBlockState(canopyLogPos).canBeReplaced()) {
                    level.setBlock(
                            canopyLogPos,
                            ACBlockRegistry.THORNWOOD_BRANCH
                                    .get()
                                    .defaultBlockState()
                                    .setValue(ThornwoodBranchBlock.FACING, upFlag ? Direction.UP : direction)
                                    .setValue(
                                            ThornwoodBranchBlock.WATERLOGGED,
                                            level.getFluidState(canopyLogPos).is(Fluids.WATER)),
                            3);
                } else {
                    Block wood = j == canopyLength - 1 || upFlag
                            ? ACBlockRegistry.THORNWOOD_WOOD.get()
                            : ACBlockRegistry.THORNWOOD_LOG.get();
                    level.setBlock(
                            canopyLogPos,
                            wood.defaultBlockState().setValue(RotatedPillarBlock.AXIS, direction.getAxis()),
                            3);
                    decorateLog(level, trunkPos, randomsource, true);
                }
            }
            ...
            if (level.getBlockState(branchPos).canBeReplaced()) {
                if (logBranches && random.nextFloat() < 0.4F) {
                    ...
                    for (int i = 0; i < bigBranchLength; i++) {
                        Block wood = i == bigBranchLength - 1
                                ? ACBlockRegistry.THORNWOOD_WOOD.get()
                                : ACBlockRegistry.THORNWOOD_LOG.get();
                        level.setBlock(
                                branchPos,
                                wood.defaultBlockState().setValue(RotatedPillarBlock.AXIS, ranDir.getAxis()),
                                3);
                ...
                level.setBlock(
                        ...
                        ACBlockRegistry.THORNWOOD_BRANCH
                                .get()
                                .defaultBlockState()
                                .setValue(ThornwoodBranchBlock.FACING, ranDir)
                                .setValue(
                                        ThornwoodBranchBlock.WATERLOGGED,
                                        level.getFluidState(branchPos).is(Fluids.WATER)),
                        3);
            }
        }
    }

    public static void generateRoot(
            WorldGenLevel level,
            BlockPos from,
            float bendChance,
            RandomSource random,
        ...
        if (level.getBlockState(rootPos).canBeReplaced()) {
            level.setBlock(
                    ...
                    ACBlockRegistry.THORNWOOD_BRANCH
                            .get()
                            .defaultBlockState()
                            .setValue(ThornwoodBranchBlock.FACING, Direction.DOWN)
                            .setValue(
                                    ThornwoodBranchBlock.WATERLOGGED,
                                    level.getFluidState(rootPos).is(Fluids.WATER)),
                    3);
        }
    }
    ...
    private static boolean canReplace(BlockState state) {
        return (state.isAir()
                        || state.canBeReplaced()
                        || state.is(ACBlockRegistry.THORNWOOD_BRANCH.get())
