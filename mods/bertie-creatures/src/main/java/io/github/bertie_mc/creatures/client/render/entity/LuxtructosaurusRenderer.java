...
import io.github.bertie_mc.creatures.client.model.LuxtructosaurusModel;
...
public class LuxtructosaurusRenderer extends MobRenderer<LuxtructosaurusEntity, LuxtructosaurusModel>
        implements CustomBookEntityRenderer {
    private static final ResourceLocation TEXTURE =
            ResourceLocation.fromNamespaceAndPath(BertieCreatures.MODID, "textures/entity/luxtructosaurus.png");
    private static final ResourceLocation TEXTURE_ENRAGED =
            ResourceLocation.fromNamespaceAndPath(BertieCreatures.MODID, "textures/entity/luxtructosaurus_enraged.png");
    private static final ResourceLocation TEXTURE_ENRAGED_GLOW = ResourceLocation.fromNamespaceAndPath(
            BertieCreatures.MODID, "textures/entity/luxtructosaurus_enraged_glow.png");
    ...
    protected RenderType getRenderType(
            LuxtructosaurusEntity mob, boolean normal, boolean translucent, boolean outline) {
        ResourceLocation resourcelocation = this.getTextureLocation(mob);
        if (translucent) {
            return RenderType.itemEntityTranslucentCull(resourcelocation);
        } else if (normal) {
            return sepia
                    ? ACRenderTypes.getBookWidget(resourcelocation, true)
    ...
    class LayerGlow extends RenderLayer<LuxtructosaurusEntity, LuxtructosaurusModel> {
        ...
        public void render(
                ...
                int packedLightIn,
                LuxtructosaurusEntity entitylivingbaseIn,
                float limbSwing,
                ...
                float netHeadYaw,
                float headPitch) {
            float enragedAlpha = ((float) (Math.sin(ageInTicks * 0.2F)) * 0.15F + 0.85F)
                    * entitylivingbaseIn.getEnragedProgress(partialTicks);
            VertexConsumer ivertexbuilder1 =
                    bufferIn.getBuffer(ACRenderTypes.getEyesAlphaEnabled(TEXTURE_ENRAGED_GLOW));
