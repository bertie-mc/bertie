...
public class VesperFlyAndHangGoal extends Goal {
    ...
    public void tick() {
        if (wantsToHang) {
            if (hangCheckIn-- < 0) {
                ...
                if (!entity.isHanging()
                        && entity.canHangFrom(entity.posAbove(), entity.level().getBlockState(entity.posAbove()))) {
    ...
    private Vec3 findFlightPos() {
        ...
        Vec3 heightAdjusted = entity.position()
                .add(
                        entity.getRandom().nextInt(range * 2) - range,
                        0,
                        entity.getRandom().nextInt(range * 2) - range);
        if (entity.level().canSeeSky(BlockPos.containing(heightAdjusted))) {
            Vec3 ground = groundPosition(heightAdjusted);
            heightAdjusted =
                    new Vec3(heightAdjusted.x, ground.y + 4 + entity.getRandom().nextInt(3), heightAdjusted.z);
        } else {
            Vec3 ground = groundPosition(heightAdjusted);
            BlockPos ceiling = BlockPos.containing(ground).above(2);
            while (ceiling.getY() < entity.level().getMaxBuildHeight()
                    && !entity.level().getBlockState(ceiling).isSolid()) {
                ceiling = ceiling.above();
            }
            float randCeilVal = 0.3F + entity.getRandom().nextFloat() * 0.5F;
            heightAdjusted =
                    new Vec3(heightAdjusted.x, ground.y + (ceiling.getY() - ground.y) * randCeilVal, heightAdjusted.z);
        }

        BlockHitResult result = entity.level()
    ...
    private boolean isOverWaterOrVoid() {
        ...
        while (position.getY() > entity.level().getMinBuildHeight()
                && entity.level().isEmptyBlock(position)) {
            position = position.below();
        }
        return !entity.level().getFluidState(position).isEmpty()
                || entity.level().getBlockState(position).is(Blocks.VINE)
                || position.getY() <= entity.level().getMinBuildHeight();
    }
    ...
    public Vec3 groundPosition(Vec3 airPosition) {
        ...
        boolean flag = false;
        while (ground.getY() < entity.level().getMaxBuildHeight()
                && !entity.level().getBlockState(ground).isSolid()
                && entity.level().getFluidState(ground).isEmpty()) {
            ground.move(0, 1, 0);
            flag = true;
        }
        ground.move(0, -1, 0);
        while (ground.getY() > entity.level().getMinBuildHeight()
                && !entity.level().getBlockState(ground).isSolid()
                && entity.level().getFluidState(ground).isEmpty()) {
            ground.move(0, -1, 0);
        }
    ...
    public Vec3 findHangFromPos() {
        ...
        int range = 14;
        for (int i = 0; i < 15; i++) {
            BlockPos blockpos1 = this.entity
                    .blockPosition()
                    .offset(random.nextInt(range) - range / 2, 0, random.nextInt(range) - range / 2);
            if (!this.entity.level().isEmptyBlock(blockpos1)
                    || !this.entity.level().isLoaded(blockpos1)) {
                continue;
            }
            while (this.entity.level().isEmptyBlock(blockpos1)
                    && blockpos1.getY() < this.entity.level().getMaxBuildHeight()) {
                blockpos1 = blockpos1.above();
            }
            if (blockpos1.getY() > entity.getY() - 1
                    && entity.canHangFrom(blockpos1, entity.level().getBlockState(blockpos1))
                    && hasLineOfToPos(blockpos1)) {
                blockpos = blockpos1;
            }
        }
    ...
    public boolean hasLineOfToPos(BlockPos in) {
        HitResult raytraceresult = entity.level()
                .clip(new ClipContext(
