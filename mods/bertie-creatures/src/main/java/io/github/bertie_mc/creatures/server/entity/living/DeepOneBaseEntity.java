...
import io.github.bertie_mc.creatures.server.entity.ai.SemiAquaticPathNavigator;
import io.github.bertie_mc.creatures.server.entity.ai.VerticalSwimmingMoveControl;
import io.github.bertie_mc.creatures.server.entity.item.InkBombEntity;
import io.github.bertie_mc.creatures.server.entity.util.DeepOneReaction;
...
public abstract class DeepOneBaseEntity extends PathfinderMob implements IAnimatedEntity {
    ...
    private static final EntityDataAccessor<Boolean> SWIMMING =
            SynchedEntityData.defineId(DeepOneBaseEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Optional<BlockPos>> ALTAR_POS =
            SynchedEntityData.defineId(DeepOneBaseEntity.class, EntityDataSerializers.OPTIONAL_BLOCK_POS);
    private static final EntityDataAccessor<Boolean> SUMMONED =
            SynchedEntityData.defineId(DeepOneBaseEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> SUMMON_TIME =
            SynchedEntityData.defineId(DeepOneBaseEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> SOUNDS_ANGRY =
            SynchedEntityData.defineId(DeepOneBaseEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public static boolean checkDeepOneSpawnRules(
        ...
        if (!level.getFluidState(blockPos.below()).is(FluidTags.WATER)) {
            ...
            boolean flag = level.getDifficulty() != Difficulty.PEACEFUL
                    && Monster.isDarkEnoughToSpawn(level, blockPos, randomSource)
    ...
    public void tick() {
        ...
        fishPitch = Mth.approachDegrees(
                fishPitch, Mth.clamp((float) pitchTarget, -1.4F, 1.4F) * -(float) (180F / (float) Math.PI), 5);
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        if (compound.contains("AltarX") && compound.contains("AltarY") && compound.contains("AltarZ")) {
            this.setLastAltarPos(
                    new BlockPos(compound.getInt("AltarX"), compound.getInt("AltarY"), compound.getInt("AltarZ")));
        }
    ...
    public void handleEntityEvent(byte b) {
        if (b == 61) {
            level().addParticle(
                            ACParticleRegistry.BIG_SPLASH.get(),
                            getX(),
                            getY() + 0.5F,
                            getZ(),
                            getBbWidth() + 0.2F,
        ...
        if (DeepOneReaction.fromReputation(prev) != newReaction) {
            ...
            if (player != null) {
                ...
                player.displayClientMessage(
                        Component.translatable("entity.bertiecreatures.deep_one.reaction_"
                                + newReaction.toString().toLowerCase(Locale.ROOT)),
                        true);
            }
        }
    }
    ...
    public boolean hurt(DamageSource damageSource, float damageValue) {
        ...
        if (sup
                ...
                && !level().isClientSide
                && !damageSource.is(ACTagRegistry.DEEP_ONE_IGNORES)) {
            int decrease = -5;
            if (!this.isAlive()) {
                decrease = -15;
            }
    ...
    public void copyTarget(LivingEntity player) {
        ...
        if (priorTarget == null || !priorTarget.isAlive()) {
            ...
            if (target != null
                    ...
                    && !(target instanceof DeepOneBaseEntity)) {
    ...
    public class HurtByHostileTargetGoal extends HurtByTargetGoal {
        ...
        protected boolean canAttack(@Nullable LivingEntity target, TargetingConditions conditions) {
            if (target instanceof Player player
                    && DeepOneBaseEntity.this.getReactionTo(player) == DeepOneReaction.HELPFUL) {
                return false;
            }
        ...
        protected void alertOthers() {
            ...
            List<? extends Mob> list =
                    this.mob.level().getEntitiesOfClass(DeepOneBaseEntity.class, aabb, EntitySelector.NO_SPECTATORS);
    ...
    private class PathNavigator extends SemiAquaticPathNavigator {
        ...
        protected double getGroundY(Vec3 vec3) {
            if (isDeepOneSwimming() || !DeepOneBaseEntity.this.isInWaterOrBubble()) {
                ...
                return this.level.getFluidState(blockpos.below()).isEmpty()
                        ? vec3.y
