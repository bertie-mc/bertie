...
import io.github.bertie_mc.creatures.server.entity.item.WaterBoltEntity;
...
public class DeepOneMageEntity extends DeepOneBaseEntity {
    ...
    public static final ResourceKey<LootTable> BARTER_LOOT = ResourceKey.create(
            net.minecraft.core.registries.Registries.LOOT_TABLE,
            ResourceLocation.fromNamespaceAndPath(BertieCreatures.MODID, "gameplay/deep_one_mage_barter"));
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(5, new RandomStrollGoal(this, 1.0D, 45) {
            ...
            public boolean canUse() {
                return !DeepOneMageEntity.this.isInWaterOrBubble()
                        && super.canUse()
                        && DeepOneMageEntity.this.getAnimation() != DeepOneMageEntity.ANIMATION_TRADE;
            }
        ...
        if (this.getAnimation() == ANIMATION_SPIN) {
            ...
            if (this.getAnimationTick() % 6 == 0) {
                ...
                for (LivingEntity entity :
                        DeepOneMageEntity.this.level().getEntitiesOfClass(LivingEntity.class, bashBox)) {
    ...
    public void startAttackBehavior(LivingEntity target) {
        ...
        if (distance > 20 + f) {
            ...
            if (strafeTarget == null || strafeTarget.distanceTo(this.position()) < 4) {
                Vec3 possible =
                        target.position().add(random.nextInt(20) - 10, random.nextInt(2), random.nextInt(20) - 10);
    ...
    private boolean isTargetBlocked(Vec3 target) {
        ...
        return this.level()
                        .clip(new ClipContext(
                                Vector3d, target, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, this))
                        .getType()
                != HitResult.Type.MISS;
    }
    ...
    public boolean startDisappearBehavior(Player player) {
        this.getLookControl()
                .setLookAt(player.getX(), player.getEyeY(), player.getZ(), 20.0F, (float) this.getMaxHeadXRot());
    ...
    public void handleEntityEvent(byte b) {
        if (b == 66) {
            for (int i = 0; i < 2 + random.nextInt(4); i++) {
                this.level()
                        .addParticle(
                                random.nextBoolean() ? ACParticleRegistry.DEEP_ONE_MAGIC.get() : ParticleTypes.DOLPHIN,
                                this.getRandomX(1F),
                                this.getRandomY(),
                                this.getRandomZ(1F),
                                0F,
            ...
            for (int i = 0; i < 13 + random.nextInt(6); i++) {
                this.level()
                        .addParticle(
                                ACParticleRegistry.DEEP_ONE_MAGIC.get(),
                                this.getRandomX(1F),
                                this.getRandomY(),
                                this.getRandomZ(1F),
            ...
            Vec3 rVec = new Vec3(0.65F, this.getBbHeight() * 0.5F + 0.15F, 0.2F)
                    .xRot(-this.getXRot() * ((float) Math.PI / 180F))
                    .yRot(-this.getYHeadRot() * ((float) Math.PI / 180F))
                    .add(deltaPos);
            Vec3 lVec = new Vec3(-0.65F, this.getBbHeight() * 0.5F + 0.15F, 0.2F)
                    .xRot(-this.getXRot() * ((float) Math.PI / 180F))
                    .yRot(-this.getYHeadRot() * ((float) Math.PI / 180F))
                    .add(deltaPos);
            this.level()
                    .addParticle(
                            ACParticleRegistry.DEEP_ONE_MAGIC.get(),
                            rVec.x + (random.nextFloat() - 0.5F) * 0.1F,
                            rVec.y + (random.nextFloat() - 0.5F) * 0.1F,
                            rVec.z + (random.nextFloat() - 0.5F) * 0.1F,
                            (random.nextFloat() - 0.5F) * 0.3F + getDeltaMovement().x,
                            1,
                            (random.nextFloat() - 0.5F) * 0.3F + getDeltaMovement().z);
            this.level()
                    .addParticle(
                            ACParticleRegistry.DEEP_ONE_MAGIC.get(),
                            lVec.x + (random.nextFloat() - 0.5F) * 0.1F,
                            lVec.y + (random.nextFloat() - 0.5F) * 0.1F,
                            lVec.z + (random.nextFloat() - 0.5F) * 0.1F,
                            (random.nextFloat() - 0.5F) * 0.3F + getDeltaMovement().x,
                            1,
                            (random.nextFloat() - 0.5F) * 0.3F + getDeltaMovement().z);
    ...
    class FlightMoveController extends MoveControl {
        ...
        public void tick() {
            parentEntity.setDeltaMovement(
                    parentEntity.getDeltaMovement().add(0, Math.sin(tickCount * 0.1) * 0.005F, 0));
            if (this.operation == MoveControl.Operation.MOVE_TO) {
                Vec3 vector3d = new Vec3(
                        this.wantedX - parentEntity.getX(),
                        this.wantedY - parentEntity.getY(),
                        this.wantedZ - parentEntity.getZ());
