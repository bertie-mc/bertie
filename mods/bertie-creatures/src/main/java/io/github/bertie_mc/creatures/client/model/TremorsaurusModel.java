...
public class TremorsaurusModel extends AdvancedEntityModel<TremorsaurusEntity> {
    ...
    public void renderToBuffer(
            PoseStack matrixStackIn, VertexConsumer bufferIn, int packedLightIn, int packedOverlayIn, int packedColor) {
        if (this.young) {
            float f = 1.5F;
            head.setScale(f, f, f);
    ...
    private void setupAnimForAnimation(
            TremorsaurusEntity entity, Animation animation, float limbSwing, float limbSwingAmount, float ageInTicks) {
        float partialTick = ageInTicks - entity.tickCount;
        if (entity.getAnimation() == TremorsaurusEntity.ANIMATION_ROAR) {
            float animationIntensity =
                    ACMath.cullAnimationTick(entity.getAnimationTick(), 1, animation, partialTick, 5, 40);
        ...
        if (entity.getAnimation() == TremorsaurusEntity.ANIMATION_SHAKE_PREY) {
            float animationIntensity =
                    ACMath.cullAnimationTick(entity.getAnimationTick(), 1, animation, partialTick, 5, 30);
    ...
    public void setupAnim(
        ...
        lfoot.rotationPointY +=
                Math.min(0, walkValue(limbSwing, limbSwingAmount, walkSpeed, -1.5F, walkDegree * 2, false));
        ...
        rfoot.rotationPointY +=
                Math.min(0, walkValue(limbSwing, limbSwingAmount, walkSpeed, -1.5F, walkDegree * 2, true));
    ...
    private float walkValue(
            float limbSwing, float limbSwingAmount, float speed, float offset, float degree, boolean inverse) {
    ...
    public Iterable<AdvancedModelBox> getAllParts() {
        return ImmutableList.of(
                body, neck, head, jaw, glasses, tail, tailTip, rarm, larm, rleg, rleg2, rfoot, rclaw1, rclaw2, rclaw3,
                lleg, lleg2, lfoot, lclaw1, lclaw2, lclaw3);
    }
