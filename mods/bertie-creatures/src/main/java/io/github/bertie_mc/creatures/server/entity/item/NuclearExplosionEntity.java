...
public class NuclearExplosionEntity extends Entity {
    ...
    public void tick() {
        ...
        if (!spawnedParticle) {
            ...
            while (particleY > level().getMinBuildHeight()
                    && particleY > this.getY() - radius / 2F
                    && isDestroyable(level().getBlockState(BlockPos.containing(this.getX(), particleY, this.getZ())))) {
                particleY--;
            }
            level().addAlwaysVisibleParticle(
                            ACParticleRegistry.MUSHROOM_CLOUD.get(),
                            true,
                            this.getX(),
                            particleY + 2,
                            this.getZ(),
                            this.getSize(),
                            isIntentionalGameDesign() ? 1.0F : 0.0F,
                            0);
        }
        if (tickCount > 40 && destroyingChunks.isEmpty()) {
            ...
            if (!level().isClientSide && !isNoGriefing()) {
                ...
                if (destroyingChunks.isEmpty()) {
                    ...
                    destroyingChunks.sort((blockPos1, blockPos2) -> Double.compare(
                            blockPos2.distManhattan(this.blockPosition()),
                            blockPos1.distManhattan(this.blockPosition())));
                } else {
            ...
            for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, killBox)) {
                ...
                if (damage > 0) {
                    ...
                    if (damage > 0) {
                        entity.hurt(
                                isIntentionalGameDesign()
                                        ? ACDamageTypes.causeIntentionalGameDesign(level().registryAccess())
                                        : ACDamageTypes.causeNukeDamage(level().registryAccess()),
                                damage);
                    }
                }
                ...
                if (!this.isIntentionalGameDesign()) {
                    entity.addEffect(new MobEffectInstance(
                            ACEffectRegistry.IRRADIATED, 48000, getSize() <= 1.5F ? 1 : 2, false, false, true));
                }
            }
        }
    }
    ...
    private void loadChunksAround(boolean load) {
        if (this.level() instanceof ServerLevel serverLevel) {
            ...
            int dist = Math.max(
                    getChunksAffected(), serverLevel.getServer().getPlayerList().getViewDistance() / 2);
            for (int i = -dist; i <= dist; i++) {
                for (int j = -dist; j <= dist; j++) {
                    if (load) {
                        BertieCreatures.TICKET_CONTROLLER.forceChunk(
                                serverLevel, this, chunkPos.x + i, chunkPos.z + j, true, true);
                    } else {
                        BertieCreatures.TICKET_CONTROLLER.forceChunk(
                                serverLevel, this, chunkPos.x + i, chunkPos.z + j, false, true);
                    }
                }
            }
        }
    ...
    private void removeChunk(int radius) {
        ...
        if (BertieCreatures.COMMON_CONFIG.nukeMaxBlockExplosionResistance.get() <= 0) {
            return;
        }
        if (dummyExplosion == null) {
            dummyExplosion = new Explosion(
                    level(),
                    null,
                    this.getX(),
                    this.getY(),
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                for (int y = 15; y >= 0; y--) {
                    ...
                    carve.set(
                            chunkCorner.getX() + x,
                            Mth.clamp(chunkCorner.getY() + y, level().getMinBuildHeight(), level().getMaxBuildHeight()),
                            chunkCorner.getZ() + z);
                    float widthSimplexNoise1 =
                            (ACMath.sampleNoise3D(carve.getX(), carve.getY(), carve.getZ(), radius) - 0.5F) * 0.45F
                                    + 0.55F;
                    double yDist = ACMath.smin(
                            0.6F - Math.abs(this.blockPosition().getY() - carve.getY()) / (float) radius, 0.6F, 0.2F);
                    double distToCenter = carve.distToLowCornerSqr(
                    ...
                    if (distToCenter <= targetRadius) {
                        ...
                        if ((!state.isAir() || !state.getFluidState().isEmpty()) && isDestroyable(state)) {
                            ...
                            canSetToFire = true;
                            if (BertieCreatures.COMMON_CONFIG.nukesSpawnItemDrops.get()
                                    && random.nextFloat() < itemDropModifier
                    ...
                    if (canSetToFire
                            ...
                            && !level().getBlockState(carveBelow).isAir()) {
    ...
    private boolean isDestroyable(BlockState state) {
        return (!state.is(ACTagRegistry.NUKE_PROOF)
                && state.getBlock().getExplosionResistance()
                        < BertieCreatures.COMMON_CONFIG.nukeMaxBlockExplosionResistance.get());
    }
