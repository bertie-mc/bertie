...
public class LuxtructosaurusMeleeGoal extends Goal {
    ...
    public void tick() {
        ...
        if (target != null && target.isAlive()) {
            ...
            if (this.luxtructosaurus.getAnimation() == SauropodBaseEntity.ANIMATION_SPEW_FLAMES
                    || this.luxtructosaurus.getAnimation() == SauropodBaseEntity.ANIMATION_LEFT_KICK
                    || this.luxtructosaurus.getAnimation() == SauropodBaseEntity.ANIMATION_RIGHT_KICK
                    || this.luxtructosaurus.getAnimation() == SauropodBaseEntity.ANIMATION_LEFT_WHIP
                    || this.luxtructosaurus.getAnimation() == SauropodBaseEntity.ANIMATION_RIGHT_WHIP) {
                luxtructosaurus.turningFast = true;
                Vec3 vec3 = target.position().subtract(luxtructosaurus.position());
                this.luxtructosaurus.yBodyRot = Mth.approachDegrees(
                        luxtructosaurus.yBodyRot, -((float) Mth.atan2(vec3.x, vec3.z)) * (180F / (float) Math.PI), 15);
            ...
            if (this.luxtructosaurus.getAnimation() == IAnimatedEntity.NO_ANIMATION) {
                if (distance < attackDistance + 4.0D) {
                    ...
                    if (random < 0.33 && this.luxtructosaurus.onGround()) {
                        ...
                        this.luxtructosaurus.playSound(
                                ACSoundRegistry.LUXTRUCTOSAURUS_ATTACK_STOMP.get(),
                                3.0F,
                                this.luxtructosaurus.getVoicePitch());
                    } else if (random < 0.66 && distance < attackDistance + 1.0D) {
                        this.luxtructosaurus.playSound(
                                ACSoundRegistry.LUXTRUCTOSAURUS_KICK.get(), 3.0F, this.luxtructosaurus.getVoicePitch());
                        this.luxtructosaurus.setAnimation(
                                this.luxtructosaurus.getRandom().nextBoolean()
                                        ? LuxtructosaurusEntity.ANIMATION_LEFT_KICK
                                        : LuxtructosaurusEntity.ANIMATION_RIGHT_KICK);
                    } else {
                        this.luxtructosaurus.playSound(
                                ACSoundRegistry.LUXTRUCTOSAURUS_TAIL.get(), 3.0F, this.luxtructosaurus.getVoicePitch());
                        this.luxtructosaurus.setAnimation(
                                this.luxtructosaurus.getRandom().nextBoolean()
                                        ? LuxtructosaurusEntity.ANIMATION_RIGHT_WHIP
                                        : LuxtructosaurusEntity.ANIMATION_LEFT_WHIP);
                    }
                } else {
                    farFlag = true;
                ...
                if ((roarCooldown == 0 && this.luxtructosaurus.getRandom().nextInt(roarRandomChance) == 0
                        || !luxtructosaurus.isEnraged()
                                && luxtructosaurus.getHealth() < luxtructosaurus.getMaxHealth() * 0.25F)) {
                    ...
                    this.luxtructosaurus.enragedFor =
                            500 + luxtructosaurus.getRandom().nextInt(200);
                    this.luxtructosaurus.setEnraged(true);
                }
            }
            if (successfulJumpCooldown <= 0
                    && navigationCheckCooldown-- < 0
                    && (luxtructosaurus.onGround() || luxtructosaurus.isInLava())) {
                ...
                if (!canReach(target) && this.luxtructosaurus.getAnimation() == IAnimatedEntity.NO_ANIMATION) {
                    ...
                    this.luxtructosaurus.playSound(
                            ACSoundRegistry.LUXTRUCTOSAURUS_JUMP.get(), 5.0F, this.luxtructosaurus.getVoicePitch());
    ...
    private boolean canReach(LivingEntity target) {
        if (target.distanceTo(this.luxtructosaurus) > 50) {
            ...
            PathResult path =
                    ((AdvancedPathNavigate) this.luxtructosaurus.getNavigation()).moveToLivingEntity(target, 1.0D);
