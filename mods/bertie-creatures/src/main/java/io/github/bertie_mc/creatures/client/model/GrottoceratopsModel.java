...
public class GrottoceratopsModel extends AdvancedEntityModel<GrottoceratopsEntity> {
    ...
    private void setupAnimForAnimation(
            GrottoceratopsEntity entity,
            Animation animation,
            float limbSwing,
            float limbSwingAmount,
            float ageInTicks) {
        float partialTick = ageInTicks - entity.tickCount;
        boolean chewing = animation == GrottoceratopsEntity.ANIMATION_CHEW_FROM_GROUND
                || animation == GrottoceratopsEntity.ANIMATION_CHEW;
        if (chewing) {
            float animationIntensity = ACMath.cullAnimationTick(
                    entity.getAnimationTick(),
                    3,
                    animation,
                    partialTick,
                    animation == GrottoceratopsEntity.ANIMATION_CHEW_FROM_GROUND ? 15 : 0);
    ...
    public void setupAnim(
        ...
        boolean showGrass = entity.getAnimation() == GrottoceratopsEntity.ANIMATION_CHEW_FROM_GROUND
                        && entity.getAnimationTick() > 15
                || entity.getAnimation() == GrottoceratopsEntity.ANIMATION_CHEW;
        ...
        larm.rotationPointY +=
                Math.min(0, ACMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -1.5F, 5F, false)) - bodyBob;
        ...
        rarm.rotationPointY +=
                Math.min(0, ACMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -1.5F, 5F, true)) - bodyBob;
        ...
        lleg.rotationPointY +=
                Math.min(0, ACMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -0.5F, 5F, true)) - bodyBob;
        ...
        rleg.rotationPointY +=
                Math.min(0, ACMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -0.5F, 5F, false)) - bodyBob;
        rleg.rotationPointZ += ACMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -0.5F, 1F, false);
        if (entity.getAnimation() != GrottoceratopsEntity.ANIMATION_MELEE_TAIL_1
                && entity.getAnimation() != GrottoceratopsEntity.ANIMATION_MELEE_TAIL_2) {
    ...
    private void articulateLegs(LegSolverQuadruped legs, float partialTick) {
        ...
        float max =
                Math.max(Math.max(heightBackLeft, heightBackRight), Math.max(heightFrontLeft, heightFrontRight)) * 0.8F;
    ...
    public void renderSpiritToBuffer(
            PoseStack poseStack,
