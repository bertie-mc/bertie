...
import io.github.bertie_mc.creatures.client.model.SplashModel;
...
public class BigSplashParticle extends Particle {
    private static final ResourceLocation TEXTURE =
            ResourceLocation.fromNamespaceAndPath(BertieCreatures.MODID, "textures/particle/splash.png");
    private static final ResourceLocation TEXTURE_OVERLAY =
            ResourceLocation.fromNamespaceAndPath(BertieCreatures.MODID, "textures/particle/splash_overlay.png");
    ...
    public void tick() {
        ...
        if (k < 5) {
            ...
            for (int j = 0; j < scale * 2 + 1; j++) {
                Vec3 sputterFrom = new Vec3(
                                (level.random.nextFloat() - 0.5F) * 0.1F * scale,
                                -0.25F,
                                (level.random.nextFloat() - 0.5F) * 0.1F * scale)
                        .add(this.x, this.y, this.z);
                this.level.addParticle(
                        ACParticleRegistry.BIG_SPLASH_EFFECT.get(),
                        sputterFrom.x,
                        sputterFrom.y,
                        sputterFrom.z,
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
        ...
        MODEL.renderToBuffer(
                ...
                net.minecraft.util.FastColor.ARGB32.colorFromFloat(alpha, colorR, colorG, colorB));
        VertexConsumer overlayconsumer =
                multibuffersource$buffersource.getBuffer(RenderType.entityTranslucent(TEXTURE_OVERLAY));
        MODEL.renderToBuffer(
                posestack,
                overlayconsumer,
                packedLight,
