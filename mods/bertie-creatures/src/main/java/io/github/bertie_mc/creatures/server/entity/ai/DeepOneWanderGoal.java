...
public class DeepOneWanderGoal extends Goal {
    ...
    public boolean canContinueToUse() {
        return goal != null
                ...
                && mob.getRandom().nextInt(200) != 0
                && !mob.isTradingLocked();
    }

    public void start() {
        groundTarget = mob.onGround()
                ? mob.getRandom().nextFloat() < 0.7F
                : mob.getRandom().nextFloat() < 0.2F;
        goal = findSwimToPos();
    }
    ...
    public boolean isTargetBlocked(Vec3 target) {
        ...
        return mob.level()
                        .clip(new ClipContext(
                                Vector3d, target, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, mob))
                        .getType()
                != HitResult.Type.MISS;
    }
    ...
    public BlockPos findSwimToPos() {
        ...
        for (int i = 0; i < 15; i++) {
            BlockPos blockPos = around.offset(
                    mob.getRandom().nextInt(range) - range / 2,
                    mob.getRandom().nextInt(range) - range / 2,
                    mob.getRandom().nextInt(range) - range / 2);

            if (mob.level().getFluidState(blockPos).is(FluidTags.WATER)
                    && !isTargetBlocked(Vec3.atCenterOf(blockPos))
                    && blockPos.getY() > mob.level().getMinBuildHeight() + 1) {
                if (groundTarget) {
                    while (groundTarget
                            && mob.level().getFluidState(blockPos.below()).is(FluidTags.WATER)
