...
import io.github.bertie_mc.creatures.client.render.entity.LuxtructosaurusRenderer;
...
public class LuxtructosaurusAshParticle extends TextureSheetParticle {
    ...
    protected LuxtructosaurusAshParticle(
            ClientLevel level, double x, double y, double z, int luxtructosaurusId, SpriteSet sprites) {
    ...
    public void tick() {
        ...
        if (!this.removed) {
            ...
            double d0 = Math.cos(Math.toRadians((double) (this.particleRandom * 60.0F)))
                    * 5.0D
                    * Math.pow((double) f1, 1.25D);
            double d1 = Math.sin(Math.toRadians((double) (this.particleRandom * 60.0F)))
                    * 5.0D
                    * Math.pow((double) f1, 1.25D);
    ...
    public void setInitialPos() {
        if (luxtructosaurusId != -1
                && level.getEntity(luxtructosaurusId) instanceof LuxtructosaurusEntity luxtructosaurus) {
            ...
            if (manager.getRenderer(luxtructosaurus) instanceof LuxtructosaurusRenderer luxtructosaurusRenderer) {
                ...
                for (int attempts = 0; attempts < 5; attempts++) {
                    ...
                    TabulaModelRenderUtils.ModelBox randomBox =
                            box.cubeList.size() > 0 ? box.cubeList.get(random.nextInt(box.cubeList.size())) : null;
                    if (randomBox != null) {
                        ...
                        Vec3 translate = translateAndRotate(box, innerOffset)
                                .yRot((float) (Math.PI - luxtructosaurus.yBodyRot * ((float) Math.PI / 180F)));
                        this.setPos(
                                luxtructosaurus.getX() + translate.x,
                                luxtructosaurus.getY() + translate.y,
                                luxtructosaurus.getZ() + translate.z);
                        return;
                    }
                }
            }
    ...
    public static class Factory implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(
                ...
                double ySpeed,
                double zSpeed) {
            LuxtructosaurusAshParticle particle =
                    new LuxtructosaurusAshParticle(worldIn, x, y, z, (int) xSpeed, spriteSet);
