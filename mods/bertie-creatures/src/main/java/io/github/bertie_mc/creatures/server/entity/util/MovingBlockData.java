...
public class MovingBlockData {
    ...
    public MovingBlockData(Level level, CompoundTag tag) {
        this(
                NbtUtils.readBlockState(level.holderLookup(Registries.BLOCK), tag.getCompound("BlockState")),
                getShapeFromTag(tag.getCompound("VoxelShape")),
                new BlockPos(tag.getInt("OffsetX"), tag.getInt("OffsetY"), tag.getInt("OffsetZ")),
                tag.contains("BlockData") ? tag.getCompound("BlockData") : null);
    }
    ...
    private static VoxelShape getShapeFromTag(CompoundTag data) {
        ...
        if (data.contains("AABBs")) {
            ...
            for (int i = 0; i < listtag.size(); ++i) {
                ...
                AABB aabb = new AABB(
                        innerTag.getDouble("BoxMinX"),
                        innerTag.getDouble("BoxMinY"),
                        innerTag.getDouble("BoxMinZ"),
                        innerTag.getDouble("BoxMaxX"),
                        innerTag.getDouble("BoxMaxY"),
                        innerTag.getDouble("BoxMaxZ"));
