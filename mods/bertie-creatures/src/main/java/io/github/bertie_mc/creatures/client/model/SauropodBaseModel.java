package io.github.bertie_mc.creatures.client.model;
...
public abstract class SauropodBaseModel<T extends SauropodBaseEntity> extends AdvancedEntityModel<T> {
    ...
    public void animate(IAnimatedEntity entity) {
        ...
        animator.rotate(
                left_Arm, (float) Math.toRadians(-20), (float) Math.toRadians(-20), (float) Math.toRadians(-20));
    ...
    private void setupAnimForAnimation(
            SauropodBaseEntity entity, Animation animation, float limbSwing, float limbSwingAmount, float ageInTicks) {
        float partialTick = ageInTicks - entity.tickCount;
        if (entity.getAnimation() == SauropodBaseEntity.ANIMATION_ROAR) {
            float animationIntensity =
                    ACMath.cullAnimationTick(entity.getAnimationTick(), 1, animation, partialTick, 5, 50);
        ...
        if (entity.getAnimation() == SauropodBaseEntity.ANIMATION_EPIC_DEATH) {
            float animationIntensity =
                    ACMath.cullAnimationTick(entity.getAnimationTick(), 1, animation, partialTick, 5, 110);
        ...
        if (entity.getAnimation() == SauropodBaseEntity.ANIMATION_SPEW_FLAMES) {
            float animationIntensity =
                    ACMath.cullAnimationTick(entity.getAnimationTick(), 1, animation, partialTick, 5, 70);
        ...
        if (entity.getAnimation() == SauropodBaseEntity.ANIMATION_EAT_LEAVES) {
            float animationIntensity =
                    ACMath.cullAnimationTick(entity.getAnimationTick(), 3, animation, partialTick, 35);
            ...
            this.jaw.rotationPointZ +=
                    animationIntensity * 2F + ACMath.walkValue(ageInTicks, animationIntensity, 0.4F, 0.5F, 1F, false);
        }
    }
    ...
    private void positionNeckAndTail(SauropodBaseEntity entity, float netHeadYaw, float headPitch, float partialTicks) {
        if (!straighten && !entity.isFakeEntity()) {
            float neckPart1Pitch =
                    (float) Math.toRadians(entity.neckPart1.calculateAnimationAngle(partialTicks, true)) * 0.5F;
            float neckPart2Pitch =
                    (float) Math.toRadians(entity.neckPart2.calculateAnimationAngle(partialTicks, true)) * 0.5F;
            float neckPart3Pitch =
                    (float) Math.toRadians(entity.neckPart3.calculateAnimationAngle(partialTicks, true)) * 0.5F;
            float tailPart1Pitch =
                    (float) Math.toRadians(entity.tailPart1.calculateAnimationAngle(partialTicks, true)) + 0.141F;
            float tailPart2Pitch =
                    (float) Math.toRadians(entity.tailPart2.calculateAnimationAngle(partialTicks, true)) + 0.076F;
            float tailPart3Pitch =
                    (float) Math.toRadians(entity.tailPart3.calculateAnimationAngle(partialTicks, true)) * 0.5F;
            float neckPart2Yaw = entity.neckPart2.calculateAnimationAngle(partialTicks, false);
            float pitchAmount = entity.getAnimation() == SauropodBaseEntity.ANIMATION_SPEW_FLAMES
                    ? 0.0F
                    : Mth.clamp(headPitch, -30, 30) / 57.295776F;
            float headApproach =
                    Mth.approachDegrees(neckPart2Yaw, entity.headPart.calculateAnimationAngle(partialTicks, false), 45F)
                            - neckPart2Yaw;
    ...
    private void animateLegWalking(
            AdvancedModelBox leg,
            AdvancedModelBox foot,
            float offset,
            float speed,
            float degree,
            float limbSwing,
        ...
        float leg1 = Math.min(
                        0,
                        ACMath.walkValue(limbSwing, limbSwingAmount, speed, Mth.PI * (offset + 0.3333F), 1F, true)
                                + 0.75F)
                * 4;
        float leg1Delayed =
                Math.min(0, ACMath.walkValue(limbSwing, limbSwingAmount, speed, Mth.PI * offset, 1F, true) + 0.75F) * 4;
        float leg1Prev = Math.min(
                        0,
                        ACMath.walkValue(limbSwing, limbSwingAmount, speed, Mth.PI * (offset + 0.6666F), 1F, true)
                                + 0.75F)
                * 4;
    ...
    private float articulateLegs(LuxtructosaurusLegSolver legs, float raiseArmsAmount, float partialTick) {
        ...
        float max = Math.max(
                        ...
                        armsArticulateAmount * Math.max(heightFrontLeft, heightFrontRight))
                * 0.75F;
