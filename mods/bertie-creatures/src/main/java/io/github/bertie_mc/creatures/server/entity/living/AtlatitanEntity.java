...
public class AtlatitanEntity extends SauropodBaseEntity implements KeybindUsingMount {
    ...
    private static final EntityDataAccessor<Optional<BlockPos>> EATING_POS =
            SynchedEntityData.defineId(AtlatitanEntity.class, EntityDataSerializers.OPTIONAL_BLOCK_POS);
    private static final EntityDataAccessor<Optional<BlockState>> LAST_EATEN_BLOCK =
            SynchedEntityData.defineId(AtlatitanEntity.class, EntityDataSerializers.OPTIONAL_BLOCK_STATE);
    private static final EntityDataAccessor<Integer> RIDEABLE_FOR =
            SynchedEntityData.defineId(AtlatitanEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Float> METER_AMOUNT =
            SynchedEntityData.defineId(AtlatitanEntity.class, EntityDataSerializers.FLOAT);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(
                4, new TemptGoal(this, 1.1D, Ingredient.of(net.minecraft.world.item.Items.WHEAT), false));
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            if (this.getAnimation() == ANIMATION_EAT_LEAVES
                    && this.getAnimationTick() > 35
                    && this.getAnimationTick() < 90) {
                BlockState lastEatenBlock = getLastEatenBlock();
                if (lastEatenBlock != null) {
                    Vec3 crumbPos = this.headPart
                            .position()
                            .add(
                    ...
                    this.level()
                            .addParticle(
                                    new BlockParticleOption(ParticleTypes.BLOCK, lastEatenBlock),
                                    crumbPos.x,
                                    crumbPos.y,
                                    crumbPos.z,
            ...
            if (this.getRideableFor() > 0 && level().random.nextInt(2) == 0 && !this.isDancing()) {
                Vec3 particlePos = this.headPart
                        .position()
                        .add(
                                (random.nextFloat() - 0.5F) * 2.0F * this.getScale(),
                ...
                this.level()
                        .addParticle(
                                ACParticleRegistry.HAPPINESS.get(),
                                particlePos.x,
                                particlePos.y,
                                particlePos.z,
                                ((double) this.random.nextFloat() - 0.5D) * 0.1D,
    ...
    public boolean onFeedMixture(ItemStack itemStack, Player player) {
        if (itemStack.is(net.minecraft.world.item.Items.GOLDEN_CARROT)) {
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!prev.consumesAction() && itemstack.is(net.minecraft.world.item.Items.GOLDEN_CARROT) && !this.isBaby()) {
            if (!itemstack.getCraftingRemainingItem().isEmpty()) {
                this.spawnAtLocation(itemstack.getCraftingRemainingItem().copy());
            }
            this.usePlayerItem(player, hand, itemstack);
            return InteractionResult.SUCCESS;
        } else if (!prev.consumesAction()
                && this.getRideableFor() > 0
                && !this.isBaby()
    ...
    public BlockPos getStandAtTreePos(BlockPos target) {
        ...
        return target.relative(dir.getOpposite(), (int) Math.floor(13 * this.getScale()))
                .atY((int) this.getY());
    }
        ...
        if (this.isPassengerOfSameVehicle(passenger)
            ...
            if (this.getAnimation() == ANIMATION_STOMP) {
                float animationIntensity =
                        ACMath.cullAnimationTick(this.getAnimationTick(), 1F, ANIMATION_STOMP, 1.0F, 0, 30);
            ...
            moveFunction.accept(
                    ...
                    this.getY() + seatOffset.y + ridingOffset - this.getLegSolverBodyOffset(),
