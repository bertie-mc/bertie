...
import io.github.bertie_mc.creatures.server.entity.living.HullbreakerEntity;
...
public class HullbreakerInspectMobGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        List<LivingEntity> list =
                entity.level().getEntitiesOfClass(LivingEntity.class, aabb, HullbreakerEntity.GLOWING_TARGET);
        if (!list.isEmpty()) {
            LivingEntity closest = null;
            for (LivingEntity mob : list) {
                if ((closest == null || mob.distanceToSqr(entity) < closest.distanceToSqr(entity))
                        && entity.hasLineOfSight(mob)
                        && !mob.is(entity)) {
                    closest = mob;
                }
            }
        ...
        if (entity.getRandom().nextInt(20) == 0 && !HullbreakerEntity.GLOWING_TARGET.test(inspectingTarget)) {
            ...
            if (staring) {
                ...
                if (isPreyWatching() && distance < 18) {
                    ...
                    Vec3 frontVision = inspectingTarget
                            .getEyePosition()
                            .add(inspectingTarget.getLookAngle().scale(12.0F));
                ...
                Vec3 circle = orbitAroundPos(
                        inspectingTarget.getEyePosition(), 12 + Math.min(0, 5 - entity.getInterestLevel()) * 3);
        ...
        if (vec31.distanceTo(vec3) > 128.0D) {
            ...
            return entity.level()
                            .clip(new ClipContext(
                                    vec3, vec31, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, lowestPrey))
                            .getType()
                    == HitResult.Type.MISS;
        }
    }
