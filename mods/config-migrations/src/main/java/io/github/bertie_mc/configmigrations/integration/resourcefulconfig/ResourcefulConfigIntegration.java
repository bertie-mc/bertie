...
public final class ResourcefulConfigIntegration {
    ...
    private ResourcefulConfigIntegration(MigrationManager migrations, Map<Selector, Target> targets) {
        this.migrations = migrations;
        this.targets = targets;
    }

    public static ResourcefulConfigIntegration load(MigrationManager migrations, Path directory, Path configDirectory) {
        ...
        for (MigrationManifest manifest : MigrationManifest.loadDirectory(directory)) {
            ...
            Target target = new Target(configDirectory.resolve(selector.config() + ".jsonc"), manifest);
            if (targets.putIfAbsent(selector, target) != null) {
                throw new ConfigMigrationException("Duplicate Resourceful Config migration target " + selector);
            }
        }
    ...
    public void migrate(String modId, String configId, Object nativeConfig, Runnable nativeSave) {
        Target target = targets.get(new Selector(modId, configId));
        if (target == null) {
            nativeSave.run();
            return;
        }
        ResourcefulConfigBridge.migrate(migrations, target.manifest(), target.file(), nativeConfig, nativeSave);
    }
