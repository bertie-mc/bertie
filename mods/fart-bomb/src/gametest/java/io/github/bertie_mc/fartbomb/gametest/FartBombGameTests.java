...
public final class FartBombGameTests {
    ...
    public static void fieryBlocksRespectTagsAndLitState(GameTestHelper helper) {
        helper.assertTrue(Blocks.MAGMA_BLOCK.defaultBlockState().is(ModTags.IGNITES_BELOW),
        ...
        helper.setBlock(relativeFeet,
                Blocks.CAMPFIRE.defaultBlockState().setValue(BlockStateProperties.LIT, false));
        helper.assertFalse(FartBomb.onFart(helper.getLevel(), FART, player),
        ...
        helper.assertTrue(FartBomb.onFart(helper.getLevel(), FART, player),
                "a lit campfire should ignite the player");
        helper.succeed();
    }
