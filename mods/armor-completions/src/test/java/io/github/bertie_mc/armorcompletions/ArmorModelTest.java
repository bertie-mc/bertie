...
class ArmorModelTest {
    ...
    @Test
    void nativeJavaModelsBakeAsHumanoidArmor() {
        var shellLegs = Modelspinyshellarmorleggings.asHumanoidModel(
                Modelspinyshellarmorleggings.createBodyLayer().bakeRoot());
        var shellBoots = Modelspinyshellarmorboots.asHumanoidModel(
                Modelspinyshellarmorboots.createBodyLayer().bakeRoot());
        var boneLegs = new BoneReptileLeggingsModel<>(
                BoneReptileLeggingsModel.createArmorLayer().bakeRoot());
        var boneBoots = new BoneReptileBootsModel<>();
    ...
    @Test
    void geckoLibBakesEachCompletedGeometryWithWearableBootBones() throws Exception {
        for (String family : java.util.List.of("bishop_of_deceit", "pyromancer_brute", "nameless_one", "necromancer")) {
            ...
            try (var stream = getClass().getResourceAsStream(file)) {
                ...
                Model raw = KeyFramesAdapter.GEO_GSON.fromJson(
                        new InputStreamReader(stream, StandardCharsets.UTF_8), Model.class);
                ...
                for (String side : java.util.List.of("Left", "Right")) {
                    assertFalse(baked.getBone("armor" + side + "Boot")
                            .orElseThrow()
                            .getCubes()
                            .isEmpty());
                    assertFalse(baked.getBone("armor" + side + "Leg")
                            .orElseThrow()
                            .getCubes()
                            .isEmpty());
                }
            ...
            try (var stream =
                    getClass().getResourceAsStream("/assets/hazennstuff/textures/armor/" + family + "_completed.png")) {
    ...
    @Test
    void everyNewInventoryModelResolvesToItsApprovedSprite() throws Exception {
        int count = 0;
            ...
            for (var type : family.missingTypes()) {
                ...
                try (var stream = getClass().getResourceAsStream(path)) {
                    ...
                    String[] texture = json.getAsJsonObject("textures")
                            ...
                            .getAsString()
                            .split(":");
                    try (var png = getClass()
                            .getResourceAsStream("/assets/" + texture[0] + "/textures/" + texture[1] + ".png")) {
