...
public final class BedRecipes {
    ...
    public static StackPredicate item(String id) {
        ...
        return new StackPredicate() {
            ...
            public boolean test(ItemStack s) {
                return !s.isEmpty()
                        && BuiltInRegistries.ITEM.getKey(s.getItem()).equals(rl);
            }
    ...
    public static StackPredicate slagPart(String material, String part) {
        return new StackPredicate() {
            ...
            @Override
            public String describe() {
                return "slag:dynamic_part{" + material + "," + part + "}";
            }
        };
    }

    private static boolean componentIs(ItemStack stack, String typeId, String expected) {
        DataComponentType<?> type = BuiltInRegistries.DATA_COMPONENT_TYPE.get(ResourceLocation.parse(typeId));
    ...
    public record BedRecipe(
            ...
            String resultId,
            int resultCount,
            String extraReturnId,
            int extraReturnCount) {
    ...
    private static Input in(String id, int count) {
        return new Input(item(id), count);
    }

    public static final List<BedRecipe> RECIPES = List.of(BedRecipe.of(
            "utrem_jar",
            in("minecraft:glass", 8),
            in("forbidden_arcanus:edelwood_planks", 1),
            null,
            false,
    ...
    public static StackPredicate tag(String tagId) {
        net.minecraft.tags.TagKey<net.minecraft.world.item.Item> key = net.minecraft.tags.TagKey.create(
                net.minecraft.core.registries.Registries.ITEM, ResourceLocation.parse(tagId));
        return new StackPredicate() {
            @Override
            public boolean test(ItemStack s) {
                return !s.isEmpty() && s.is(key);
            }
