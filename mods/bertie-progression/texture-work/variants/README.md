...
`abyssal_core_hunter.png` is the exception, and knowingly so — **it is a hand
export and it will not track its generator.** That generator lives outside this
repository, in the workspace's gitignored `custom-textures/`, because that is
where the workspace `CLAUDE.md` says custom-texture work is authored. Nothing
regenerates this file, so if the abyssal core is redrawn, re-export it or drop
it. The same gap applies to the *shipped* `abyssal_core.png`, which unlike
storm, desert and weeping_eye has no generator in `texture-work/` at all.
