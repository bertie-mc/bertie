 ...
 * {@code config/berlordscarving-armor-overrides.json} mapping a carving material to explicit
...
public final class ArmorOverrides {
    ...
    private static Map<CarvingMaterial, Map<ArmorKind, String>> load() {
        ...
        Path file = FMLPaths.CONFIGDIR.get().resolve("berlordscarving-armor-overrides.json");
        ...
        try (Reader reader = Files.newBufferedReader(file)) {
            ...
            if (!out.isEmpty()) {
                Carving.LOGGER.info("berlordscarving: loaded armor output overrides for {}", out.keySet());
            }
        } catch (Exception e) {
            Carving.LOGGER.error("berlordscarving: failed to read {} - ignoring overrides", file, e);
        }
        return out;
    }
