...
def find_artifact(project: Path) -> Path:
    location = (project / "build" / "libs").resolve(strict=True)
    if not location.is_dir():
        raise RuntimeError(f"Gradle artifact directory is not a directory: {location}")
