...
public final class TestScheduler {
    ...
    static void start(Minecraft minecraft, Runnable testRunner) {
    ...
    static boolean canAcceptServerTasks(MinecraftServer target) {
        return target == server && SERVER_REGISTERED.get() && serverCanAcceptTasks;
    }

    static void requireNoServerRunning() {
    ...
    static synchronized void recordTerminalFailure(Throwable failure) {
    ...
    static void throwRecordedFailure() {
    ...
    static void runOnClient(Runnable action) {
        computeOnClient(() -> {
            action.run();
            return null;
        });
    ...
    static <T> T computeOnClient(Supplier<T> action) {
    ...
    static void runOnServer(MinecraftServer target, Runnable action) {
    ...
    static <T> T computeOnServer(MinecraftServer target, Supplier<T> action) {
    ...
    private static <T> T dispatch(Supplier<T> action, TaskTarget target) {
    ...
    static void rethrowFailure(Throwable failure) {
        ...
        throw new IllegalStateException("A Minecraft thread task failed", failure);
    }
    ...
    static final class DispatchedTask<T> {
        ...
        DispatchedTask(TaskTarget target, Supplier<T> action) {
            this.target = target;
            this.action = action;
        }
